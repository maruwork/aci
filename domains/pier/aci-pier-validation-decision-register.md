...
- generic template authority is in `shared/runtime/templates/`, `shared/report/templates/`
...
| 2026-06-12 | ci_14.py (detectors/) | 全体 | CI14_PLAINTEXT_SECRET | 実装変更 | ドキュメント修正 | `scan_plaintext_secrets` は常に `critical` を使用 | 2026-05-26 エントリの「テストファイルは WARN に severity downgrade」記述は現行実装と不一致。現行 `ci_14.py` は `SEVERITY_CRITICAL` を固定使用し、パス依存の severity 分岐なし | テストも `critical` を期待しており実装破壊なし。文書側が過去の設計意図を残した状態 | テスト確認済み: `test_aci_detector_fixtures.py` は `CI14_PLAINTEXT_SECRET` に `critical` を期待 | パスによる severity 分岐は現在実装されていない。将来追加する場合は `CONFIDENCE_MEDIUM` (secrets は pattern-based) と組み合わせて再評価する | CI14 doc sync 2026-06-12 | supersedes-prior-entry |
