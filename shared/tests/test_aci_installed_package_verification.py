...
def test_release_gate_checks_fall_back_to_installed_metadata(monkeypatch, tmp_path: Path) -> None:
    fake_package_module = SimpleNamespace(__version__="0.1.8")
    fake_scan_module = SimpleNamespace(ACI_TOOL_VERSION="0.1.8")
    ...
    monkeypatch.setattr(
        verify.importlib.metadata,
        "version",
        lambda name: version_calls.append(name) or "0.1.8",
    )
