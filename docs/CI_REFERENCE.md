...
| CI-14 | Security Neglect | `eval`/`exec`; `subprocess(shell=True)`; plaintext secret (AST); plain `http://` (docstrings/comments skipped); unsafe YAML / deserialization; bounded supply-chain drift checks | high (secret/http/supply-chain: medium) | partial (bandit S); ACI finds more |
...
The current common-shelf CI-14 supply-chain detector intentionally covers only:
...
such as `pyproject.toml`, `poetry.lock`, `package-lock.json`, `pnpm-lock.yaml`,
